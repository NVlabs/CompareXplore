...
def encode_X_torch_multi_modality(mmo, d_node, d_edge, preprocessors, gname, vname, task=FLAGS.task, cached_edge_index=None, cached_edge_attr=None):
    dict_programl = encode_X_torch_programl(mmo.g_programl, d_node['n_dict_programl'], d_edge['e_dict_programl'],
                                            preprocessors['preprocessors_programl'], gname, vname, return_Data=False, task=task, cached_edge_index=cached_edge_index, cached_edge_attr=cached_edge_attr)
    dict_src_code = encode_X_torch_src_code(d_node['n_dict_src_code'], d_edge['e_dict_src_code'],
                                            preprocessors['preprocessors_src_code'], gname, vname, return_Data=False, task=task)
    ...
    if FLAGS.combine_fashion == 'share_final_MLPs':
        ...
        if FLAGS.pc_links:
            ...
            for node, d in sorted(mmo.pc_links.items()):
                ...
                for i, (chunk_id, token_id_local, token_global_id_in_chunks) in enumerate(
                    ...
                    saver.log_info_at_most(
                        ...
                        100)
                    """
            data.pos_node_ids = torch.tensor(pos_node_ids, dtype=torch.int64)

            # saver.log_info(f'data.pos_node_ids.dtype={data.pos_node_ids.dtype}')

            if data.pos_node_ids.dtype == torch.float32:
                raise RuntimeError(f'Something is wrong; Check config.py')

            data.pos_chunk_ids = torch.tensor(pos_chunk_ids, dtype=torch.int64)
            data.pos_token_ids = torch.tensor(pos_token_ids, dtype=torch.int64)
            ...
            if FLAGS.pc_links_aug in ['all_line_sw', 'all_line_swp', 'all_line_swp_grease', 's_grease']:
                data.pos_token_sw_type_in_chunks = torch.tensor(pos_token_sw_type_in_chunks, dtype=torch.int64)
            if FLAGS.node_token_interaction:
                data.pos_token_global_ids_in_chunks = torch.tensor(pos_token_global_ids_in_chunks, dtype=torch.int64)


                # from model_node_token_interact import _check_node_ids

                # for t in pos_token_global_ids_in_chunks:
                #     if not (0 <= t < 32 * 64):
                #         raise RuntimeError(f'Wrong here! data.x_src_code.shape={data.x_src_code.shape}')

                # saver.log_info_once(f'data.x_src_code.shape={data.x_src_code.shape} data.gname={data.gname}')
                # _check_node_ids(data.pos_token_global_ids_in_chunks, data.x_src_code.view(data.x_src_code.shape[0] * data.x_src_code.shape[1], 1))
