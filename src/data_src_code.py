...
from encoder_openai import OpenAIEncoder
...
def create_tokenizer():
    ...
    if FLAGS.code_encoder == 'codebert':
        ...
        # xxx = tokenizer.cls_token
        # print()
    elif FLAGS.code_encoder == 'codellama/CodeLlama-7b-hf':
        tokenizer = AutoTokenizer.from_pretrained("codellama/CodeLlama-7b-hf")
        # tokenizer = AutoTokenizer.from_pretrained('replit/replit-code-v1_5-3b', trust_remote_code=True)
 
        if tokenizer.cls_token is None: # decoder-only LLM may set this to None
            ...
            new_words += [tokenizer.mask_token]
        MAX_LEN = 256
    elif FLAGS.code_encoder == 'TheBloke/CodeLlama-7B-GPTQ':
        tokenizer = AutoTokenizer.from_pretrained(FLAGS.code_encoder, use_fast=True)
        ...
        # Actually, the tokenizer does not matter for OpenAI. 
        # Just use codet5-small's so that the later pre-processing code can run fine.
        # Eventually just need to pass the original text (in string) to OpenAI's client.
        tokenizer = RobertaTokenizer.from_pretrained('Salesforce/codet5-small')
        MAX_LEN = 256  # can be larger, actually
...
SOURCE_FILES = None
if FLAGS.sequence_modeling:# or FLAGS.pairwise_class:  # even if we use HAPR, pairwise_class = True would still be able to let us do comparison# and FLAGS.force_regen:
...
def create_code_encoder():
    if FLAGS.code_encoder == 'codebert':
       ...
       model = LlamaForCausalLM.from_pretrained("codellama/CodeLlama-7b-hf",  load_in_8bit=True, torch_dtype="auto", device_map=saver.get_device()) # must set device_map when load_in_8bit=True; otherwise will throw some error on cuda:0 vs cuda:<another>
    #    saver.log_info(f'saver.accelerator.device={saver.accelerator.device}')
    ...
    #    config.attn_config['attn_impl'] = 'triton'
        ...
        encoder = AutoModelForCausalLM.from_pretrained(FLAGS.code_encoder,
                                             device_map="auto",
...
def read_source_code(gexf_file, gname=None, print_msg=True):
    if gname is None:
    ...
    if SOURCE_FILES is None:
        raise RuntimeError(
            f'SOURCE_FILES is None -- check if config.py is correct (Maybe load a wrong model? Maybe just set sequence_modeling=True?) SOURCE_FILES')
    c_li = []
...
def _encode_feat_dict_sequence_pure_seq(g, preprocessors, point, pc_links, g_programl, n_dict_programl=None):
    if 'encoders' not in preprocessors:
        preprocessors = preprocessors['preprocessors_src_code'] 
    num_tokens = preprocessors['counters']['num_tokens']
    num_matched_pairs = preprocessors['counters']['num_matched_pairs']

    text, text_to_use, text_simplified = get_text_representation_of_design(g, point)
    ...
    if hasattr(FLAGS, 'pc_links') and FLAGS.pc_links:
        ...
        if FLAGS.pc_links_aug in ['all_line_swp_grease', 'grease', 's_grease']:
            ...
            for t in cls_token_gids:
                if not (0 <= t < 32 * 64):
                    raise RuntimeError(f'Wrong here!')
    ...
    saver.log_info_once(f'@@@@@ dara src code 699 {g.gname} has nodes: {g_new.number_of_nodes()}')


    return {'g_new': g_new,
            'X_pragma_trans': {}, 'X_pragma_nodes': X_pragma_nodes, 'text_to_use': text_to_use}, {}



def get_text_representation_of_design(g, point):
    assert g.number_of_nodes() == 1
    text = g.nodes[0]['text']
    ...
    for key, value in sorted_point:
        ...
        if c >= 2:
            saver.log_info_once(f'Kernel {g.gname} has {key} appearing more than once ({c} times) (okay but double check)')
        # assert c == 1
    ...
    if FLAGS.data_repr in ['full', 'penhance']:
        ...
        assert False
    return text, text_to_use, text_simplified
...
def encode_X_torch_src_code(d_node, d_edge, preprocessors, gname, vname, return_Data=True, task=FLAGS.task):
    if 'encoders' not in preprocessors:
        preprocessors = preprocessors['preprocessors_src_code'] 
    ...
    # saver.log_info_once(f'##### ds 902 {gname} has nodes: {g_new.number_of_nodes()}')
    ...
    for node, data in sorted(g_new.nodes(data=True)):
        ...
        assert tokens is not None and None not in tokens

        # try:
        tokens_ids = tokenizer.convert_tokens_to_ids(tokens)
        # except Exception as e:
        #     saver.log_info(e)
        #     saver.log_info(f'token={tokens}')
        #     saver.log_info(f'{tokens[0] is None}')
    ...
    # saver.log_info_once(f'##### {gname} has nodes: {g_new.number_of_nodes()} len(tokens_ids_list)={len(tokens_ids_list)} X_node={X_node.shape}')
    ...
    if FLAGS.data_repr == 'ast':
        ...
        if FLAGS.ptrans:
            ...
            X_ast_edge_labels = coo_to_sparse(X_ast_edge_labels, FLAGS.device).to_dense()
        else:
    ...
    if FLAGS.code_encoder == 'OpenAI':
       text_to_use =  d_node['text_to_use']
    else:
        text_to_use = None # save disk space
    ...
    if return_Data:
        ...
        if task == 'regression':
            return Data(
                ...
                text_to_use=text_to_use, # for OpenAI to encode
