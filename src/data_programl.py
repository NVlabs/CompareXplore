...
from utils import check_any_in_str, NON_OPT_PRAGMAS, WITH_VAR_PRAGMAS, coo_to_sparse, load, create_edge_index, print_g, get_flag_handling_HARP, HARP_model_loaded
...
def encode_feat_dict_programl(g, preprocessors, point=None):
    if 'counters' not in preprocessors:
        preprocessors = preprocessors['preprocessors_programl'] # tricky code; due to force_data_dir, we possbily load from a multi_modality dataset which contains preprocessors_programl (see data_multi_modality.py's encode_feat_dict_multi_modality() for details)
    ...
    for nid, (node, ndata) in enumerate(sorted(g.nodes(data=True))):  # TODO: node ordering
        ...
        if 'pseudo' in ndata['text']:
            ...
            ## for pragma as MLP
            pragma_scope = get_flag_handling_HARP(FLAGS, 'pragma_scope')
            if pragma_scope == 'block':
                ...
                saver.log_info(f'{pragma_scope} is not supported')
    ...
    if not FLAGS.multi_modality:
        if not HARP_model_loaded(FLAGS):
...
def _coo_to_sparse(coo):
    ...
    i = torch.LongTensor(indices).to(FLAGS.device)
    v = torch.FloatTensor(values).to(FLAGS.device)
    shape = coo.shape
...
def encode_X_torch_programl(g, d_node, d_edge, preprocessors, gname, vname, return_Data=True, task=FLAGS.task,   
                            cached_edge_index=None, cached_edge_attr=None):
    """
    x_dict is the returned dict by _encode_X_dict()
    """
    if 'encoders' not in preprocessors:
        preprocessors = preprocessors['preprocessors_programl'] 
    ...
    if FLAGS.no_pragma:
        ...
        s = f'Example encoded feature dimensions for the {task} task:\nX_ntype.shape[1]={X_ntype.shape[1]}\nX_ptype.shape[1]={X_ptype.shape[1]}\nlen(X_numeric[0])={len(X_numeric[0])}\nX_itype.shape[1]={X_itype.shape[1]}\nX_ftype.shape[1]={X_ftype.shape[1]}\nX_btype.shape[1]={X_btype.shape[1]}'
        saver.log_info_at_most(s, msg_type=f'{task}_encoding_dim', times=1)
        # zzz = np.array(X.todense())
        X = coo_to_sparse(X, FLAGS.device)
        X_node = X.to_dense()

    if cached_edge_attr is None:
        ...
        X_edge = coo_to_sparse(X_edge, FLAGS.device)
        X_edge = X_edge.to_dense()
    else:
        saver.log_info_once('cached X_edge')
        X_edge = cached_edge_attr # reuse the cache

    if cached_edge_index is None:
        edge_index = create_edge_index(g)
    else:
        saver.log_info_once('cached edge_index')
        edge_index = cached_edge_index # reuse the cache
