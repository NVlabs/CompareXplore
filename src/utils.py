...
def check_nx_version():
    ...
    if not nxva.startswith('2'):
        # if nxvg != nxva:
        raise RuntimeError(f'Wrong networkx version! Need >=2 instead of {nxva}')
...
def proc_filepath(filepath, ext='.klepto'):
    if type(filepath) is not str:
        raise RuntimeError(f'Did you pass a file path to this function? type(filepath)={type(filepath)}; filepath={filepath}')
    ...
    for k, v in loss_dict_.items():
        if k not in target_list:
            if k not in loss_dict:
                loss_dict[k] = 0 # lenient code: handle any loss components that are NOT defined previously but somehow added by model forward pass eg pairwise class loss components
...
def deduce_MLP_dimensions(D, final_D, reduce_factor):
    """
    Generates a list of integers, gradually changing from an initial dimension D 
    to a final dimension final_D, using the provided reduce factor.

    Args:
        D (int): The initial dimension.
        final_D (int): The desired final dimension.
        reduce_factor (float): Factor to apply during reduction or increase.

    Returns:
        list: A list of integers representing the intermediate dimensions, excluding D and final_D.

    Examples:
        print(deduce_MLP_dimensions(576, 2, 2))  # Output: [288, 144, 72, 36, 18, 9, 4]
        print(deduce_MLP_dimensions(64, 1024, 0.5))  # Output: [128, 256, 512]
        print(deduce_MLP_dimensions(1000, 10, 3))  # Output: [333, 111, 37, 12]

    """

    assert reduce_factor > 0, "reduce_factor must be greater than 0"
    assert D > 0 and final_D > 0, "D and final_D must be positive"
    assert (reduce_factor > 1 and D > final_D) or (reduce_factor < 1 and D < final_D), \
        "D to final_D must be decreasing if reduce_factor > 1 or increasing if reduce_factor < 1"

    if D == final_D:
        return []  # Boundary case: No change needed

    step = 1 if reduce_factor > 1 else -1  # Determine direction of change
    values = []

    current_D = int(D / reduce_factor)  # Start with the first intermediate value
    while (step > 0 and current_D > final_D) or (step < 0 and current_D < final_D):
        values.append(current_D)
        current_D = int(current_D / reduce_factor)   
