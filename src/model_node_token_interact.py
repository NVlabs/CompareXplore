...
def interact_the_two_modalities(inputs_embeds, nti_objs, layer_id):
    ...
    if FLAGS.code_encoder == 'codet5':
        assert 0 <= layer_id <= 6, f'layer_id={layer_id}'
    else:
...
def _forward_node_token_interaction(inputs_embeds_src_code, nti_objs, layer_id):
    ...
    # _check_node_ids(nti_objs['data'].pos_token_global_ids_in_chunks, inputs_embeds_src_code_flat)
    ...
    # _check_node_ids(pos_token_global_ids_in_chunks.view(1, -1), x)
    # saver.log_info(f'num_nodes_total={num_nodes_total}')

    shifted_token_ids = pos_token_global_ids_in_chunks.view(1, -1) + num_nodes_total


    # _check_node_ids(node_ids, x)
    # _check_node_ids(shifted_token_ids, x)
    ...
    if FLAGS.interleave_GNN_transformer:
        ...
        # _check_node_ids(edge_index, x)
        # _check_node_ids(edge_index_token_to_node, x)
...
#    xxx = input('end')
    ...
    # try:
    # saver.log_info(f'{edge_index.dtype}')
...
def _check_node_ids(node_ids, x):
    for nid in node_ids.flatten().detach().tolist():
        if not (0 <= nid < x.shape[0]):
            raise RuntimeError(f'Wrong nid {nid}; x.shape[0]={x.shape[0]} and node_ids={node_ids}')