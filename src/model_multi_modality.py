...
from model_node_token_interact import _check_node_ids
...
class MultiModalityNet(Model):
    ...
    def _init_programl_plus_src_code(self, dataset):
        ...
        # saver.log_info(f'bert_model created {timer.time_and_clear()}')
        ...
        if hasattr(FLAGS, "two_modalities_interact_start_layer") and -1 < FLAGS.two_modalities_interact_start_layer:
            if FLAGS.code_encoder == 'codet5': 
                for name, param in self.bert_model.named_parameters():
                    ...
                    if 'block' not in name and 'embed_token' not in name:
                    #print(name)
                        continue
                    if 'embed_token' in name:
                        ...
                        #print(name)
                            pass
                # saver.log_info(f'two_modalities_interact_start_layer {timer.time_and_clear()}')
        ...
        # saver.log_info(f'node_attention {timer.time_and_clear()}')
        ...
        if FLAGS.combine_fashion == 'share_final_MLPs': # default choice
            ...
            # saver.log_info(f'conv_layers {timer.time_and_clear()}')
...
#                input_D_to_base = 2 * D
            if FLAGS.node_attention:
                scale = FLAGS.separate_T + FLAGS.separate_P + FLAGS.separate_pseudo + FLAGS.separate_icmp
                if scale== 0:
                    scale = 1 # TODO: check
                input_D_to_base = D * scale
            ...
            # saver.log_info(f'base_decoder {timer.time_and_clear()}')
            ...
            # saver.log_info(f'decoder MLPs {timer.time_and_clear()}')
            ...
            if hasattr(FLAGS, 'node_token_interaction') and FLAGS.node_token_interaction:
                if hasattr(FLAGS, 'node_token_interact_start_layer'):
                    ...
                    freeze_layers = 0
                self.node_token_interaction_GNN = create_node_token_interaction_module(D, get_code_encoder_num_layers() 
                - freeze_layers)

                # saver.log_info(f'node_token_interaction_GNN {timer.time_and_clear()}')
                ...
                if hasattr(FLAGS, 'project_node_embs_before_align') and FLAGS.project_node_embs_before_align:
                    ...
                    # saver.log_info(f'project_node_embs_before_align {timer.time_and_clear()}')
                ...
                if FLAGS.interleave_GNN_transformer:
                    ...
                    # saver.log_info(f'interleave_GNN_transformer {timer.time_and_clear()}')
            ...
            if FLAGS.pc_links:
                ...
                # saver.log_info(f'pc_links {timer.time_and_clear()}')
        ...
        # saver.log_info(f'_init_programl_plus_src_code done {timer.time_and_clear()}')
    ...
    def _forward_programl_plus_src_code(self, data, tvt):
        ...
        if FLAGS.combine_fashion == 'share_final_MLPs':  # TODO handle edge_attr_src_code
            ...
            # _check_node_ids(data.pos_token_global_ids_in_chunks, x_src_code.view(x_src_code.shape[0] * x_src_code.shape[1], 1))
        ...
        if FLAGS.combine_fashion == 'share_final_MLPs':
            ...
            if not FLAGS.feed_p_to_tf:
                ...
                # _check_node_ids(nti_objs['data'].pos_token_global_ids_in_chunks, x_src_code.view(x_src_code.shape[0] * x_src_code.shape[1], 1))
                ...
                _, x_src_code_full = self._apply_bert(x_src_code, data, additional_x=out_embed_programl_repeat,
                                                      nti_objs=nti_objs)
                x_bert = x_src_code_full['last_hidden_state'].float()
            ...
            total_loss += self._apply_target_MLPs_with_loss(self.decoder_shared, input_to_final_MLPs, data, out_dict,
                                              loss_dict, 'normal')
            ...
            if FLAGS.pc_links: # default false
                if FLAGS.node_token_alignment_loss:
                    ...
                    loss_pos = \
                        F.binary_cross_entropy_with_logits(
                            pred_pos, torch.ones_like(pred_pos, dtype=torch.float, device=pred_pos.device))
                    loss_neg = \
                        F.binary_cross_entropy_with_logits(
                            pred_neg, torch.zeros_like(pred_neg, dtype=torch.float, device=pred_neg.device))
                ...
                if FLAGS.gs_alignment_loss: #default false
                    ...
                    if FLAGS.batch_size == 1 or num_data1 == 1:
                        ...
                        gs_loss_pos = \
                            F.binary_cross_entropy_with_logits(
                                gs_pred_pos, torch.ones_like(gs_pred_pos, dtype=torch.float, device=gs_pred_pos.device))
                        gs_loss_neg = \
                            F.binary_cross_entropy_with_logits(
                                gs_pred_neg, torch.zeros_like(gs_pred_neg, dtype=torch.float, device=gs_pred_neg.device))
    ...
    def _merge_to_get_input_to_shared_GNNs(self, out_programl, out_src_code, batch_programl, batch_src_code):
        ...
        for i in range(FLAGS.batch_size):
            ...
            if FLAGS.multi_glevel_embs:
                mask_programl.append(torch.tensor([0] * p.shape[0], device=out_programl.device))
                mask_programl.append(torch.tensor([-10000] * s.shape[0], device=out_programl.device))
                mask_src_code.append(torch.tensor([-10000] * p.shape[0], device=out_programl.device))
                mask_src_code.append(torch.tensor([0] * s.shape[0], device=out_programl.device))
